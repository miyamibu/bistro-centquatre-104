...
- [ ] Vercel Production env に LINE 6 値が投入済
...
2. LINE アプリ内ブラウザで https://<本番ドメイン>/booking を開く
   ...
   - もしくは LIFF URL `https://liff.line.me/<NEXT_PUBLIC_LIFF_BOOKING_ID>` を開けば LIFF アプリ枠で表示される
...
- 事前LINE通知登録もない場合、`Reservation.lineUserId` は **null**
...
| 症状 | 主な原因 | 確認ポイント |
|---|---|---|
| `/booking` に LINE ボタンが出ない | `NEXT_PUBLIC_LIFF_BOOKING_ID` 未投入 / 再 deploy 未実施 | Vercel env と最新 deployment の build log |
| `/line/link` で LIFF 初期化できない | `NEXT_PUBLIC_LIFF_LINK_ID` 未投入 / LIFF Endpoint URL 違い | Vercel env と LINE Developers の Link LIFF 設定 |
...
## 後付け LINE 連携の実機テスト手順 (v2)

### 前提

- `LINE_LINK_TOKEN_PEPPER` を Vercel に設定済み
- migration `20260529150000_line_link_and_notification_ledger` 適用済み
- `/line/link` ページが LIFF エンドポイントとして登録済み

### テスト A: 予約後のリンク (token flow)

1. LIFF 連携ボタンを押さずに通常予約を送信する
2. 予約完了パネルに「LINEで前日通知を受け取る」ボタンが表示されることを確認
3. ボタンを押して `/line/link?t=xxx` ページに移動
...
1. 前日リマインド対象の予約を後付け連携する
2. 即時リマインドメッセージが届くこと
3. 翌日の cron が同じ予約に二重送信しないこと (NotificationEvent.status = SENT で skip)

### テスト D: follow/unfollow

1. 公式アカウントを友だち追加 → `LineFriend.friendshipStatus = FRIEND` が DB に記録されること
2. 友だち解除 → `BLOCKED` に更新されること
