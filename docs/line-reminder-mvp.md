...
```env
NEXT_PUBLIC_LIFF_BOOKING_ID=...  # /booking の liff.init で使う。公開値。
NEXT_PUBLIC_LIFF_LINK_ID=...     # /line/link の liff.init と後付け連携URLで使う。公開値。
LINE_LOGIN_CHANNEL_ID=...        # ID token verify の client_id。
LINE_CHANNEL_ACCESS_TOKEN=...    # Messaging API push 用。
LINE_CHANNEL_SECRET=...          # Webhook署名検証用。
LINE_LINK_TOKEN_PEPPER=...       # 連携トークン/電話番号ハッシュ用。32文字以上。
```
...
- 旧 `LIFF_ID` / `NEXT_PUBLIC_LIFF_ID` は使わず、予約フォーム用の `NEXT_PUBLIC_LIFF_BOOKING_ID` と後付け連携用の `NEXT_PUBLIC_LIFF_LINK_ID` に分ける。
- `LINE_LOGIN_CHANNEL_ID` を追加する。
- 2 つの `NEXT_PUBLIC_LIFF_*` はフロントで参照できる形にする。
...
- `NEXT_PUBLIC_LIFF_BOOKING_ID` がない場合は予約フォームのLINE通知ボタンを非表示またはdisabledにする。
