...
- `src/lib/line.ts`
- `src/lib/line-customer-link.ts`
- `src/app/api/cron/remind/route.ts`（既存 alias の維持確認用、もし新規扱いなら含める）
- `src/app/api/line/webhook/route.ts`
- `src/app/api/line/customer-link/route.ts`
- `prisma/migrations/20260511120000_add_line_reminder_fields/migration.sql`（ディレクトリごと）
- `prisma/migrations/20260529150000_line_link_and_notification_ledger/migration.sql`（ディレクトリごと）
...
- `tests/line-customer-link-route.test.ts`
...
git add prisma/migrations/20260529150000_line_link_and_notification_ledger
git add src/lib/env.ts src/lib/env-public.ts src/lib/line.ts src/lib/line-customer-link.ts
...
git add src/app/api/line/webhook/route.ts src/app/api/line/customer-link/route.ts
git add vercel.json
git add tests/line-verify.test.ts tests/line-webhook.test.ts tests/line-helpers.test.ts
git add tests/line-customer-link-route.test.ts
...
2. Vercel env 6 つ投入                        ← line-reminder-vercel-env.md
3. 本番 DB migration 適用 + 独立カラム確認    ← line-reminder-db-migration.md §6.3
...
- `prisma migrate status` が `Database schema is up to date!` を返すだけでは **不十分**。`_prisma_migrations` テーブルは対象 DB を間違えると別 DB の状態を答えるため、`up to date` 表示でも本番 DB に実カラムが無いケースが起こりうる（実際に過去 1 回発生）。
- Step 4（新コード deploy）に進む前に、必ず次の SQL を本番 DB に対して read-only で実行し、3 カラム揃って返ることを目視確認する:
  ```sql
  SELECT column_name
  ...
  ```
  期待: `lineReminderSentAt` / `lineReminderStatus` / `lineReminderError` の 3 行。
- 3 行未満なら deploy / 再デプロイを **停止**。`/booking` 復旧宣言も行わない。手順詳細と DIRECT_URL 手入力時の対象 DB 確認は [line-reminder-db-migration.md](line-reminder-db-migration.md) §6.3 (b), (c) 参照。
...
- [ ] `NEXT_PUBLIC_LIFF_BOOKING_ID` が反映され、ブラウザ DOM に「LINE」ボタンが描画されている
- [ ] `NEXT_PUBLIC_LIFF_LINK_ID` が反映され、`/line/link` で LIFF 初期化できる
...
`LINE_CHANNEL_ACCESS_TOKEN` を外すと `hasLineMessagingEnv()` が false を返し、cron は `SKIPPED_LINE_SETUP` を返すだけになる。通常予約は影響を受けない。LIFF ボタンを完全に消したいなら `NEXT_PUBLIC_LIFF_BOOKING_ID` / `NEXT_PUBLIC_LIFF_LINK_ID` も外す。
