...
const DETECTION_RULES = [
  {
    name: 'raw DELETE Reservation',
    regex: /DELETE\s+FROM\s+(?:"Reservation"|Reservation\b)/gi,
  },
  {
  ...
  {
    name: 'raw DELETE BusinessDay',
    regex: /DELETE\s+FROM\s+(?:"BusinessDay"|BusinessDay\b)/gi,
  },
  {
  ...
  {
    name: "prisma reservation.delete",
    regex: /\.reservation\.delete\s*\(/gi,
  },
  {
    name: "prisma reservation.deleteMany",
    regex: /\.reservation\.deleteMany\s*\(/gi,
  },
  {
    name: "prisma privateBlockAuditLog.delete",
    regex: /\.privateBlockAuditLog\.delete\s*\(/gi,
  },
  {
  ...
  {
    name: "prisma businessDay.delete",
    regex: /\.businessDay\.delete\s*\(/gi,
  },
  {
    name: "prisma businessDay.deleteMany",
    regex: /\.businessDay\.deleteMany\s*\(/gi,
  },
  {
...
function walkFiles(rootPath, dirPath, out) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      walkFiles(rootPath, absolutePath, out);
      continue;
    }

    const extension = path.extname(entry.name);
    if (!ALLOWED_FILE_EXTENSIONS.has(extension) || shouldSkipRelativePath(relativePath)) {
      continue;
    }
...
function collectRegexMatches(regex, source) {
  regex.lastIndex = 0;
...
export function scanSource(relativePath, source) {
  ...
  for (const rule of DETECTION_RULES) {
    const matches = collectRegexMatches(rule.regex, source);
    if (matches.length === 0) {
      continue;
    }
...
export function scanWorkspace(rootPath = process.cwd(), scanRoots = DEFAULT_SCAN_ROOTS) {
  ...
  for (const root of scanRoots) {
    ...
    walkFiles(rootPath, absoluteRoot, files);
  }

  const violations = [];
  for (const file of files) {
    const source = fs.readFileSync(file.absolutePath, "utf8");
    violations.push(...scanSource(file.relativePath, source));
  }
