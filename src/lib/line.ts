 ...
 * Returns true only on a 200 from the profile endpoint.
 */
export async function canPushToLineUser(lineUserId: string): Promise<boolean> {
  if (!LINE_USER_ID_REGEX.test(lineUserId)) return false;
  if (!hasLineMessagingEnv()) return false;
  ...
  try {
    ...
    logWarn("line.canPush.network_failed", {
      context: {
        message: truncateError(error instanceof Error ? error.message : String(error)),
      },
    });
